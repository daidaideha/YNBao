...
public class VideoTypeActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		list_data = new ArrayList<VideoUnit>();
	...
	private void initHeader() {
		...
		tv_title.setText(getString(R.string.tv_header_video_type));
	...
	private void initBodyer() {
		...
		mGridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				...
				bundle.putString("vid", list_data.get(position).getVid());
				new IntentToOther(VideoTypeActivity.this,
	...
	private class GetDataTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			// TODO Auto-generated method stub
			super.onPreExecute();
			pd = new ProgressDialog(VideoTypeActivity.this);
		...
		protected Void doInBackground(Void... pragm) {
			...
			String requery = "";
			if (type == 0) {
				requery = comFunction.getDataFromServer("get_video_list", 
						paramsList, VideoTypeActivity.this);
			} else {
				requery = comFunction.getDataFromServer("get_video_search_list", 
						paramsList, VideoTypeActivity.this);
			}
			...
			try {
				...
				if (code.equals(HttpCode.SERVICE_SUCCESS)) {
					...
					for (int i = 0; i < jArray_data.length(); i++) {
						...
						unit.setVid(jArray_data.getJSONObject(i).getString("vid"));
						unit.setThumb(jArray_data.getJSONObject(i).getString("thumb"));
		...
		protected void onPostExecute(Void result) {
			...
			if (message != null) {
				if (code.equals(HttpCode.SERVICE_SUCCESS)) {
					...
					comFunction.toastMsg(message, VideoTypeActivity.this);
				}
			} else {
				comFunction.toastMsg(getString(R.string.toast_net_link),
						VideoTypeActivity.this);
			}
			if (pd.isShowing()) {
				pd.dismiss();
			}
