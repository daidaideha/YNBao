 ...
 * 
 */
public class MyCollectKnowledgeActivity extends Activity implements
	...
	private MyCollectKnowledgeAdapter adapter;
	private GetDataTask getDataTask;
	private int pageCount = 1;
	private ArrayList<KnowledgeUnit> list_data;
	...
	private void initView() {
		...
		adapter = new MyCollectKnowledgeAdapter(this);
	...
	private class GetDataTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... arg0) {
			String requery = comFunction.getDataFromServer("get_fav_my_thread",
					paramsList, MyCollectKnowledgeActivity.this);
			...
			try {
				...
				if (code.equals(HttpCode.SERVICE_SUCCESS)) {
					...
					for (int i = 0; i < jArray_questions.length(); i++) {
						KnowledgeUnit question = new KnowledgeUnit(
		...
		protected void onPostExecute(Void result) {
			...
			if (message != null) {
				if (code.equals(HttpCode.SERVICE_SUCCESS)) {
					...
					comFunction.toastMsg(message, MyCollectKnowledgeActivity.this);
				}
			} else {
				comFunction.toastMsg(getString(R.string.toast_net_link),
						MyCollectKnowledgeActivity.this);
			}
