...
import com.innouni.yinongbao.adapter.MyCollectProductAdapter;
 ...
 * 
 */
public class MyCollectProductActivity extends Activity implements
	...
	private ArrayList<ExhibitionUnit> list_data;
	...
	private void initView() {
		...
		adapter = new MyCollectProductAdapter(this);
	...
	private class GetDataTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... arg0) {
			String requery = comFunction.getDataFromServer(
					"get_fav_my_product", paramsList,
					MyCollectProductActivity.this);
			...
			try {
				...
				if (code.equals(HttpCode.SERVICE_SUCCESS)) {
					...
					for (int i = 0; i < jArray_questions.length(); i++) {
						ExhibitionUnit question = new ExhibitionUnit(
		...
		protected void onPostExecute(Void result) {
			...
			if (message != null) {
				if (code.equals(HttpCode.SERVICE_SUCCESS)) {
					...
					comFunction
							.toastMsg(message, MyCollectProductActivity.this);
				}
			} else {
