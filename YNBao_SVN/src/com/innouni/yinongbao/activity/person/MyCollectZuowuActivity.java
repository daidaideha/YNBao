...
import com.innouni.yinongbao.adapter.MyCollectKnowledgeAdapter;
import com.innouni.yinongbao.adapter.MyCollectZuoWuAdapter;
import com.innouni.yinongbao.unit.HttpCode;
import com.innouni.yinongbao.unit.knowledge.KnowledgeUnit;
import com.innouni.yinongbao.view.PullToRefreshView;
import com.innouni.yinongbao.view.PullToRefreshView.OnFooterRefreshListener;
 ...
 * 
 */
public class MyCollectZuowuActivity extends Activity implements
	...
	private MyCollectZuoWuAdapter adapter;
	private GetDataTask getDataTask;
	private int pageCount = 1;
	private ArrayList<ZuoWu> list_data;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_my_collect_zuowu);
		sPreferences = new sPreferences(this);
		list_data = new ArrayList<ZuoWu>();
		initHeader();
		initView();
	}
	...
	private void initView() {
		...
		listView = (GridView) findViewById(R.id.list_exper);
		...
		adapter = new MyCollectZuoWuAdapter(this);
	...
	private class GetDataTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... arg0) {
			String requery = comFunction.getDataFromServer("get_fav_my_type",
					paramsList, MyCollectZuowuActivity.this);
			...
			try {
				...
				if (code.equals(HttpCode.SERVICE_SUCCESS)) {
					...
					if (jArray_questions == null) {
						return null;
					}
					for (int i = 0; i < jArray_questions.length(); i++) {
						ZuoWu question = new ZuoWu(
		...
		protected void onPostExecute(Void result) {
			...
			if (message != null) {
				if (code.equals(HttpCode.SERVICE_SUCCESS)) {
					...
					comFunction.toastMsg(message, MyCollectZuowuActivity.this);
				}
			} else {
				comFunction.toastMsg(getString(R.string.toast_net_link),
						MyCollectZuowuActivity.this);
			}
